...
export default function SummariesClient({ taskId }: { taskId: string }) {
  ...
  const [expandedTaskFileIds, setExpandedTaskFileIds] = useState<Set<string>>(new Set());
  ...
  function toggleDetails(taskFileId: string) {
    setExpandedTaskFileIds((current) => {
      ...
      if (next.has(taskFileId)) {
        next.delete(taskFileId);
      } else {
        next.add(taskFileId);
      }
      return next;
        ...
        {taskFilePagination.paginatedItems.map((taskFile) => {
          ...
          return (
            <article key={taskFile.id} className="rounded-md border border-slate-200 bg-white p-4 shadow-sm">
              <div className="flex flex-wrap items-start justify-between gap-4">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <button
                    ...
                    onClick={() => toggleDetails(taskFile.id)}
                    type="button"
                  >
                    {isExpanded ? "收起详情" : "详情"}
                  </button>
              ...
              {!summary ? <p className="mt-4 text-sm text-slate-500">暂无摘要</p> : null}

              {summary && isExpanded ? (
                <div className="mt-4 grid gap-4">
                  <p className="break-all font-mono text-xs text-slate-500">{taskFile.id}</p>
